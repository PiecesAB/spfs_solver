...
namespace SuperPowerSolver
{
    ...
    class PlayerMultiplier
    {
        ...
        {
            return Math.Max(Multiplier(StatLabel.Endurance), Math.Max(Multiplier(StatLabel.Strength), Multiplier(StatLabel.Psychic)));
        }
    ...
    class AncientItem
    {
        ...
        public int multiple = 1; // when players have multiple of an item
    }

    class ItemPack
    {
        public string name;
        public List<AncientItem> items = new List<AncientItem>();

        private double totalChance = -1;
        public AncientItem PickItem()
        {
            if (totalChance < 0)
            {
                totalChance = 0;
                for (int i = 0; i < items.Count; ++i) { totalChance += items[i].chance; }
            }
            double r = GameInfo.rand.NextDouble() * totalChance;
            int chosen = 0;
            double sumulus = items.Count - 1;
            for (int i = 0; i < items.Count; ++i) {
                sumulus += items[i].chance;
                if (chosen < sumulus) { chosen = i; break; }
            }
            return items[chosen];
        }
    }
    ...
    class Player
    {
        ...
        public double PowerMultiplier(TransformationLabel transf)
        {
            ...
            return mainMultiplier.HighestMultiplier() * rm * tm * fm;
        }
